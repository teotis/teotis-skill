...
Teotis 的公开 Codex skill 集合，当前聚焦结构性架构分析、务实遗留系统更新、可审阅技术报告和可复用 agent 规划工作流。
...
**效果：** 诊断遗留系统更新和技术债现代化，找出真正限制演进能力的主约束，并设计带稳定性护栏、采纳经济学和明确 owner 的可回滚试点路径。  
**适配场景：** 适合不能停业务、不能一把重写、还需要兼顾兼容性、交付节奏、运维责任和组织采纳的现代化决策。
...
| `renewal-architect` | 遗留系统更新与可回滚现代化 | 93 / 100 | 强在主约束诊断、试点决策契约、采纳经济学和稳定性优先的演进路径。 |
...
- `renewal-architect` 面向真实约束下的遗留系统演进：保护不能破坏的部分，试验可以安全变化的部分，用可回滚试点把争论变成足以决策的证据。
