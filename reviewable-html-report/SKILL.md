...
description: Use when an already-formed analysis, audit, plan, or artifact review needs formal HTML report mechanics such as Mermaid/topology views, review cards, local feedback persistence, or exportable review notes.
---
...
Build the reusable review presentation layer for dense technical reports. This skill does not perform the domain analysis itself and does not redesign the reader's comprehension path; it turns an already-formed analysis, plan, audit, comparison, or artifact review into a self-contained HTML report that is easy to read, inspect, annotate, and hand back to another agent.

Use it as the report companion for skills such as `abstraction-architect`, `renewal-architect`, and `analyze-success` when they already own the report schema and need an interactive HTML review surface.
...
1. Confirm that the input already has domain conclusions and a report schema. If the main task is to reorganize complex material for understanding, use a comprehension-first HTML skill instead.
...
5. Generate a self-contained HTML file unless the report is too large; for large reports, create a local bundle with sibling assets and an obvious index. If `references/report_base.md` is unavailable, use a minimal fallback shell and do not block report delivery.
