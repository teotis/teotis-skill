...
A good abstraction is not valid merely because it is more general. Strong rewrite candidates need a falsifiable **Candidate Proof Route** with the dimensions relevant to their consequence; exploratory candidates may remain partial and explicitly provisional. A non-gating **Abstraction Fitness Score** (clear examples, tool yield, cross-context links, practical sufficiency under bounded evidence and migration capacity, hot-path value, and a tiny complete loop) is optional decision support, not an absolute gate; do not manufacture evidence or reject a useful candidate only because one dimension is weak.

The default deliverable is the smallest complete decision surface: direct chat, a short Markdown note, or a durable report. Produce interactive HTML only when interactivity materially lowers review cost. Markdown is the default durable source when an artifact is useful; one-screen delivery is a preference, not a truncation rule. By default, this skill performs analysis only. It MUST NOT modify production code, tests, configuration, migrations, or infrastructure without separate explicit authorization; transition-planning depth should match the consequence and reversibility of the change.

## Capability Load

Separate **task fit** from **load depth** (authority **1A**: self-probe before Light/Skip; decision-relevant uncertainty may require Full; hard gate before finish).
...
Regardless of entry source, invocation authorizes the chosen mode within its safety and mutation boundaries. Cover the evidence faces that can change the decision; do not run low-yield workflow steps solely because the mode permits them. Narrow or upgrade depth when task evidence, consequence, or user goals warrant it.
...
- **Fallback:** only after the Report Upgrade Gate selects HTML, use the `reviewable-html-report` capability or repo-local report base. If unavailable, use bundled `references/fallback.html` to deliver self-contained static HTML that preserves the core conclusion, left collapsible scroll-spy TOC, stable section IDs, evidence appendix, Mermaid source fallback, and non-persistent feedback.
...
- **Decision triage before abstraction:** compare plausible lower-cost alternatives before escalating to a new structure. Use `local deletion wins`, `boundary repair wins`, or `structural candidate worth testing` as useful shorthand, not a mandatory three-way form.
- **Candidate proof route:** match family, difference, preservation, future deformation, and falsifier evidence to the strength and consequence of the recommendation. An exploratory candidate may remain explicitly partial; a rewrite-ready claim may not.
...
4. Deliver the smallest complete decision surface: intervention level, leading candidate or no-escalation reason, key evidence, coverage debt, and smallest next step.
5. When a durable artifact helps, write Markdown. Add HTML only when the Report Upgrade Gate is met and the interaction changes review quality; use shared report mechanics when available, but do not make browser behavior a completion requirement.
...
- **HTML is an optional review surface:** generate it when interactivity materially lowers review cost, the user requests it, or durable team review needs it. Include only the evidence, comparison, visuals, and controls that change the review; do not make browser behavior or a fixed section inventory a completion requirement.
- **Markdown is the default durable artifact:** use it when evidence needs persistence, handoff, or review. When HTML is also produced, keep evidence IDs and conclusions aligned.
...
- Recommended candidates include a proportionate proof route; abstraction fitness is optional decision support and never a forced score or veto.
- Include rejected or deferred abstractions when relevant.
- State the intervention level and why escalation is or is not warranted; the triage labels `local deletion wins`, `boundary repair wins`, and `structural candidate worth testing` are optional shorthand.
